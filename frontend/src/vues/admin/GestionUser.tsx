...
export default function GestionUser({navigation}){
    const [search, setSearch] = useState('')
    const [select, setSelect] = useState(null)
    const [visible, setVisible] = useState(false)
    const [visibleDes, setVisibleDes] = useState(false)

    const {data, loading, error, getData} = useGet(`/admin/user?page=1&limit=10`)
    ...
    const { 
        ...
        loadMore,
        refresh
    } = useGetAllPage(`/admin/user`);
    const allUsers = allUserData?.user || [];
    ...
    return(
        ...
        <View style={styles.container}>
            <HeaderComponent title="Gestion des utilisateurs" icone="user" />
            ...
            <ButtonComponent
                onPress={()=>{
                    navigation.navigate('gestion-medecin')
                    
                }}
                style={styles.jhg}
            >
                <Icon nom={'doctor'} taille={20} color={'black'} />
                <Text>Gérer les médécins</Text>
            </ButtonComponent>
